...
Each value is that module's curator params slice. The AVS injects the on-chain `policyParams` blob VERBATIM as the Rego `data.params` document and does NOT unwrap this manifest envelope, so a composite gate reads a pack's slice at `data.params.params.<short-pack-id>.<field>` — the doubled `params` is Rego's `data.params` (the whole on-chain blob) plus this manifest's own inner `params` map. Plain dot notation, NOT bracket-on-slashes. (Contrast `data.wasm.<short-pack-id>`, which is the AVS-MERGED oracle WASM output and sits at the top level — it is not part of this on-chain blob.)
...
- `params` keys use the short pack id derived via `shortPackIdFromModuleId(module.id)` (everything before the first `/`) — readable Rego access (`data.params.params.vaultsfyi.field`, matching `data.wasm.vaultsfyi.field` for WASM output) and symmetric with the Phase 0 namespacing convention. Note the doubled `params` - the AVS injects the on-chain policyParams (the full envelope) verbatim as `data.params`, so pack params are nested.

The encoder (`encodeCompositeParams`) rejects packs where two modules derive the same short id — that would make `data.params.params.<shortId>` ambiguous in Rego. `KNOWN_PACK_IDS` mechanically enforces uniqueness across the published-pack universe.
...
**`params` is bijective with the oracle module set, plus ONE reserved key.** Every module in `modules[]` has exactly one `params[shortId]` entry, and every `params` key must be a short pack id present in the module set — with a single sanctioned exception, the reserved `_policy` key (below). The encoder rejects any other `params` key that no module covers, and a module with no params entry.

**Policy-level params: the `_policy` key.** A value the composite's Rego needs that no oracle owns — a bound vault address allowlist, a cap ceiling, a fee-bps ceiling, a composite-wide threshold — is a **policy-level** param. It lives under the reserved `_policy` slice: a composite declares a `policyParamsSchema` (zod) on `defineComposite`, `encodeCompositePolicyPack` validates + emits `params._policy`, and the gate reads it at `data.params.params._policy.<field>`. The `_policy` JSON Schema is derived from the same zod at `defineComposite` (one source of truth), and `generateCompositePinnedSchema(composite)` pins it into the on-chain schema so the pinned schema matches what the encoder writes. `_policy` cannot collide with an oracle slice — its leading underscore is disallowed for oracle short ids (which must start with a lowercase letter). Retune a policy-level value with `setParams` (no Rego edit, no re-pin). Prefer this over a hardcoded Rego constant, which forces a `--regen` + redeploy to change and is an easy target-mismatch footgun.
...
Shipped in `@newton-xyz/policy-pack-shared` (`composite-manifest.ts` + `composite-introspect.ts`): the manifest type, `decodeManifest` / `isCompositeManifest`, `encodeCompositeParams` (byte producer, validates each `params[shortId]` against the module's `paramsSchema` before emitting), `introspectComposite` (multicall with sequential fallback), and the error classes above. The codegen (`scripts/generate-bindings.ts`) enforces the `_manifest` key reservation — a pack whose `params_schema.json` declares a top-level `_manifest` property fails at regen.

## Decisions

Resolved during implementation; the shipped format follows these:

- **Magic bytes** are 4-char ASCII (`"NPM1"`) — grep-able and editor-inspectable in JSON.
- **`policyDataAddress`** is EIP-55 checksummed, matching viem call returns.
- **`wasmCid`** is CIDv1 base32-lower, matching `INewtonPolicyData.getWasmCid()` and the Pinata upload pipeline.
- **`_manifest` does not carry `chainId`/`env`** — the cell is implied by the on-chain Shield the depositor queries.
- **Missing `params[shortId]` is strict** — every module in `modules[]` MUST have a corresponding `params` entry (even `{}`); a gap throws `MalformedManifestError`. Catches partial-write bugs.
