...
if [[ ${#snapshots[@]} -eq 0 ]]; then
  echo "no <pack>/dist/last_deploy.json snapshots found — run \`pnpm run upload <pack>\` then \`pnpm run deploy <pack> --env <env> --chain <chainId>\` first" >&2
  exit 1
fi
...
// any of them. A mismatch means deploy.sh ran the pack against a different
// env, then someone called sync with the wrong --env (or a stale
// `<pack>/dist/last_deploy.json` from a prior cell got picked up without
// being overwritten by a successful deploy on the requested env).
...
for (const path of snapPaths) {
  ...
  if (!s.env) {
    missingEnv.push(`${path} (snapshot has no \`env\` field — was it written by an old deploy script? Re-run \`pnpm run deploy\` for this pack to rewrite under the new schema.)`);
    continue;
  }
  if (s.env !== env) {
...
if (mismatches.length || missingEnv.length) {
  ...
  console.error("Fix: re-run \`pnpm run deploy <pack> --env <correct-env> --chain <chainId>\`");
