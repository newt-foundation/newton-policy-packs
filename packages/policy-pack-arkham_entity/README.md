# `@newton-xyz/policy-pack-arkham_entity`

Entity-aware spending gate: tiers a transaction limit by Arkham entity attribution, denying prohibited risk tags, unapproved entity categories, low-confidence attribution, and unlabelled destinations above an introductory cap

Typed TypeScript bindings for the Newton **arkham_entity** policy pack. Generated from the AVS-side artifacts at [`/arkham_entity/`](../../arkham_entity/) in this repo.

## Install

```bash
pnpm add @newton-xyz/policy-pack-arkham_entity
```
...
The `src/*` files are generated. Edit the upstream JSON schemas under [`/arkham_entity/`](../../arkham_entity/) and run `pnpm gen:bindings` from the repo root to regenerate.
