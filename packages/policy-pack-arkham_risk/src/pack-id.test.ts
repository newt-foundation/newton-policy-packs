...
const POLICY_JS_PATH = resolve(__dirname, "../../../arkham_risk/policy.js");
...
describe("PACK_ID drift check", () => {
	it("PACK_ID literal in policy.js matches PACK_NAME from metadata.ts", () => {
		...
		assert.equal(
			literalPackId,
			PACK_NAME,
			`PACK_ID drift: arkham_risk/policy.js has \`PACK_ID = "${literalPackId}"\` ` +
	...
	it("PACK_NAME matches the package folder name", () => {
		assert.equal(
			PACK_NAME,
			"arkham_risk",
			`PACK_NAME = "${PACK_NAME}" but the canonical folder is \`arkham_risk\`. ` +
				"`metadata.ts` is auto-generated from `policy_metadata.json` — " +
