...
Explainable risk gate: denies direct or near-hop exposure to curator-configured severe risk categories, escalates distant exposure only when materially large or recent, tolerates configured dust, and names the offending transaction path in the deny reason

Typed TypeScript bindings for the Newton **arkham_risk** policy pack. Generated from the AVS-side artifacts at [`/arkham_risk/`](../../arkham_risk/) in this repo.

## Install

```bash
pnpm add @newton-xyz/policy-pack-arkham_risk
```
...
The `src/*` files are generated. Edit the upstream JSON schemas under [`/arkham_risk/`](../../arkham_risk/) and run `pnpm gen:bindings` from the repo root to regenerate.
