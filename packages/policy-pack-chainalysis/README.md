...
The generated `src/*` files (everything except `pack.ts` and `prepare-query.ts`) are emitted from the upstream JSON schemas. Edit the schemas under [`/chainalysis/`](../../chainalysis/) and run `pnpm gen:bindings` from the repo root to regenerate.
