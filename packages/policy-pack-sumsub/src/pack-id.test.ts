...
const POLICY_JS_PATH = resolve(__dirname, "../../../sumsub/policy.js");
...
describe("PACK_ID drift check", () => {
	it("PACK_ID literal in policy.js matches PACK_NAME from metadata.ts", () => {
		...
		assert.equal(
			literalPackId,
			PACK_NAME,
			`PACK_ID drift: sumsub/policy.js has \`PACK_ID = "${literalPackId}"\` ` +
	...
	it("PACK_NAME matches the package folder name", () => {
		assert.equal(
			PACK_NAME,
			"sumsub",
			`PACK_NAME = "${PACK_NAME}" but the canonical folder is \`sumsub\`. ` +
				"`metadata.ts` is auto-generated from `policy_metadata.json` — " +
