...
/**
 * Composite-policy view of the sumsub pack. Pass to `defineComposite(...)`
 * (Phase 2 — see `docs/composite-policies.md`) when stacking sumsub with
 * other packs in one Shield. Strict subset of the `PolicyPack` above —
 * shares the same `id`, schemas, and deployments.
 */
export const sumsubOracleModule: OracleModule<Params, WasmArgs, Secrets> =
	oracleModuleFromPack(sumsub);
