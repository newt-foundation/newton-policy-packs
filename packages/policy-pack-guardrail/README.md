...
| `prepareQuery`, `PrepareQueryOptions` | `prepare-query.ts` | Populates `vaultAddress` from `PrepareQueryArgs.subject` and `chainId` from `publicClient.chain.id`; optional `protocolId` via the options bag; honors the `dataSourceChainId` / `dataSourceSubject` testing overrides. |
