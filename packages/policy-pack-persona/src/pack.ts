...
/**
 * Composite-policy view of the persona pack. Pass to `defineComposite(...)`
 * (Phase 2 — see `docs/composite-policies.md`) when stacking persona with
 * other packs in one Shield. Strict subset of the `PolicyPack` above —
 * shares the same `id`, schemas, and deployments.
 */
export const personaOracleModule: OracleModule<Params, WasmArgs, Secrets> =
	oracleModuleFromPack(persona);
