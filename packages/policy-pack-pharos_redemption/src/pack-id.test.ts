...
const POLICY_JS_PATH = resolve(__dirname, "../../../pharos_redemption/policy.js");
...
describe("PACK_ID drift check", () => {
	it("PACK_ID literal in policy.js matches PACK_NAME from metadata.ts", () => {
		...
		assert.equal(
			literalPackId,
			PACK_NAME,
			`PACK_ID drift: pharos_redemption/policy.js has \`PACK_ID = "${literalPackId}"\` ` +
	...
	it("PACK_NAME matches the package folder name", () => {
		assert.equal(
			PACK_NAME,
			"pharos_redemption",
			`PACK_NAME = "${PACK_NAME}" but the canonical folder is \`pharos_redemption\`. ` +
				"`metadata.ts` is auto-generated from `policy_metadata.json` — " +
