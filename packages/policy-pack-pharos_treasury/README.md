# `@newton-xyz/policy-pack-pharos_treasury`

Risk-adjusted stablecoin treasury gate: admits a stablecoin only when there is no active depeg, stress is below threshold, the redemption route is approved, and exit capacity comfortably exceeds the resulting position

Typed TypeScript bindings for the Newton **pharos_treasury** policy pack. Generated from the AVS-side artifacts at [`/pharos_treasury/`](../../pharos_treasury/) in this repo.

## Install

```bash
pnpm add @newton-xyz/policy-pack-pharos_treasury
```
...
The `src/*` files are generated. Edit the upstream JSON schemas under [`/pharos_treasury/`](../../pharos_treasury/) and run `pnpm gen:bindings` from the repo root to regenerate.
